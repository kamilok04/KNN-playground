...
class MTree:
    def __init__(self, node_size=4, KNN_size=4):
    ...
    def KNN_search(self, query, k=None):
        ...
        while pq:
            ...
            if dist > d_k:
                continue
            d_k = self.KNN_node_search(node, query, k, d_k)
            
        return [x[1] for x in self.neighbours]

    def KNN_node_search(self, node, query, k, d_k):
        pq = []
        if not node.is_leaf:
            ...
            for entry in node.entries:
                dist = self.compute_distance(entry.obj, query)
                if dist <= d_k:
                    d_k = self.NN_update( (dist, entry.obj), k)
        
        # Process non-leaf nodes
        while pq and pq[0][0] <= d_k:
            dist, child = heapq.heappop(pq)
            d_k = self.KNN_node_search(child, query, k, d_k)
            
        return d_k

    def NN_update(self, new_entry, k):
        distance, obj = new_entry
        insert_pos = bisect.bisect_left([x[0] for x in self.neighbours], distance)
        self.neighbours.insert(insert_pos, (distance, obj))
        
        if len(self.neighbours) > k:
            self.neighbours = self.neighbours[:k]
        
        return self.neighbours[-1][0] if len(self.neighbours) >= k else float('inf')
    ...
    def insert(self, obj, node=None):
        ...
        if not node.is_leaf:
            ...
            # Leaf node insertion
            if not node.is_full():
                node.entries.append(self.Entry(obj))
            else:
                self.split(node, obj)

    def split(self, node, new_obj):
        # Create new node
        new_node = MTree.Node(node_size=self.node_size, is_leaf=node.is_leaf)
    ...
    def compute_distance(self, obj1, obj2):
        if obj1 is None or obj2 is None:
            return 0
        dist = 0
        for i in range(len(obj1)):
            dist += (obj1.iloc[i] - obj2.iloc[i])**2
        return dist**0.5